...
await page.getByText("VALENDO!").waitFor({ state: "visible", timeout: 4_000 });
await page.waitForSelector(".game-card", { state: "visible", timeout: 6_000 });
await page.getByRole("button", { name: "ACERTOU" }).dblclick();
await page.waitForFunction(() => JSON.parse(localStorage.getItem("nao-fala:game:v1") ?? "{}").actionHistory?.length === 1);
...
assert.equal(JSON.parse(saved!).actionHistory.length, 1, "Toque duplo nao pode pontuar duas vezes");
...
assert.equal(await page.getByRole("button", { name: "Jogar agora" }).isVisible(), true);
assert.equal(await page.getByRole("button", { name: "Continuar partida" }).count(), 0);
assert.equal(await page.evaluate(() => localStorage.getItem("nao-fala:game:v1")), null);
